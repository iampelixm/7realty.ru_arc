...
class Item extends Model
{
    ...
    protected $fillable = ['price', 'user_id', 'name', 'description', 'type_id', 'type_order', 'added_at', 'special', 'square', 'latitude', 'longitude', 'address', 'all_rooms', 'bed_rooms', 'bath_rooms', 'active', 'option', 'residence_id', 'area_id', 'offer_index', 'video_url', 'slug', 'remark', 'meta_description', 'page_title'];
...
//print_r(json_decode($this->option));
        ...
        $options=Option::whereIn('id', array_keys($this->option))->get();
        foreach($options as $option)
        {
            $item=(object) $option->toArray();
            $item->value=$this->option[$option->id];
            // dd($this->option);
            if(!$option->slug)
            {
                $option->slug=Str::slug($option['name'], '_');
                $option->save();
                $option->refresh();
            }
            $result[$option->slug]=$item;
        }

        return $result;

        // foreach ($this->option as $option_id=>$option_value) {
    ...
    public function getAllOptionsAttribute()
    {
        ...
        foreach($this->type->options as $type_option)
        {
            ...
            if(!isset($this->options[$type_option->slug]))
            {
                $new=(object) $type_option->toArray();
                $new->value='';
                $out[]=$new;
            }
            else
            {
                $out[]=$this->options[$type_option->slug];
            }
        }
        return $out;
    }
