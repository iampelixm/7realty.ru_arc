...
function createMyModalLink(element)
{
	...
	if(title == null) title = "";
	if(body == null) body = "";
	if(href != null) actionYes = function(){openLink(href)};
...
function createMyModal(data)
{
	...
	*/
	...
	function getModalBtns(btns)
	{
		...
		for(let i =0; i < btns.length; i++) block.appendChild(getBtn(btns[i]));
		return block;
		...
		function getBtn(btn)
		{
			...
			for(let i =0; i < btnClass.length; i++) button.classList.add(btnClass[i]);
			if(typeof btn.onclick == "string") button.onclick = function() {eval(btn.onclick)};
...
document.querySelectorAll('img').forEach((el)=>{if(!el.src && 'src' in el.dataset !== -1 ) el.src = el.dataset.src});}
