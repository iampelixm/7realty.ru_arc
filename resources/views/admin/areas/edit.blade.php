...
@section('content')
    <div class="ibox-title">
        <h2>{{ __('admin.areas_edit') }}</h2>
    </div>
    <div class="ibox-content">
        <form action="{{ route('admin.areas.update', $area->id) }}" enctype="multipart/form-data" method="POST"
            role="form">
            @csrf
            @method('PUT')
            ...
            <div class="form-group">
                ...
                <div class="row">
                    <div class="col-lg-6">
                        <input type="text" class="form-control" placeholder="Slug" value="{{ $area->slug ?? '' }}"
                            disabled="">
                    </div>
                    <div class="col-lg-6">
                        Изменить url
                            ...
                            @if ($area->coordinates != null)
                                @foreach ($area->coordinates as $item)
                                    <tr>
                                        <td>{{ $indexvalue }}</td>
                                        <td>
                                            <input type="number" step="0.000001" required class="form-control"
                                                id="latitude{{ $indexvalue }}"
                                                name="coordinates[{{ $indexvalue }}][latitude]"
                                                value="{{ $item['latitude'] }}">
                                        </td>
                                        <td>
                                            <input type="number" step="0.000001" required class="form-control"
                                                id="longitude{{ $indexvalue }}"
                                                name="coordinates[{{ $indexvalue }}][longitude]"
                                                value="{{ $item['longitude'] }}">
                                        </td>
                                        <td>
                                            <input type="text" required class="form-control" id="name{{ $indexvalue }}"
                                                name="coordinates[{{ $indexvalue }}][name]"
                                                value="{{ $item['name'] }}">
                                        </td>
                                        <td>
                                            <button type="button" class="btn btn-outline btn-danger"
                                                onclick="deleterow('valuerow', {{ $indexvalue }})">Удалить</button>
                                        </td>
                                    </tr>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        function addvalue() {
            ...
            var inputname = "<input type=\"text\" value=\"\" class=\"form-control\" id=\"latitude" + indexvalue +
                "\" name=\"coordinates[" + indexvalue + "][name]\">";

            var inputlatitude = "<input type=\"number\" step=\"0.000001\" required class=\"form-control\" id=\"latitude" +
                indexvalue + "\" name=\"coordinates[" + indexvalue + "][latitude]\">";

            var inputlongitude = "<input type=\"number\" step=\"0.000001\" required class=\"form-control\" id=\"longitude" +
                indexvalue + "\" name=\"coordinates[" + indexvalue + "][longitude]\">";

            var delbutton =
                "<button type=\"button\" class=\"btn btn-outline btn-danger\" onclick=\"deleterow('valuerow', " +
                indexvalue + ")\">Удалить</button>";

            var str = "<tr id=\"valuerow" + indexvalue + "\"><td>" + indexvalue + "</td><td>" + inputlatitude +
                "</td><td>" + inputlongitude + "</td><td>" + inputname + "</td><td>" + delbutton + "</td></tr>";
