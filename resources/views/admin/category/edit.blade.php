    ...
    <div class="ibox-content">
        <form action="{{ route('admin.categories.update', $category->id) }}" enctype="multipart/form-data" method="POST"
            ...
            <div class="form-group">
                ...
                <div class="row">
                    <div class="col-lg-6">
                        <input type="text" class="form-control" placeholder="Slug" value="{{ $category->slug ?? '' }}"
                            disabled="">
                    </div>
                    <div class="col-lg-6">
                        Изменить url
                    ...
                    <select class="js-example-basic-multiple form-control" name="maincategories[]" multiple="multiple">
                        @foreach ($categorys as $cat)
                            @if ($cat->id != $category->id)
                                <option value="{{ $cat->id }}" @if (in_array($cat->id, $maincategories)) selected @endif>
                                    {{ $cat->name }}</option>
                            @endif
                        @endforeach
                    </select>
