...
@section('content')
    ...
    <div class="ibox-content">
        <div class="table-responsive">
            @if($item->images != null)
                <table class="table table-striped">
                    ...
                    @foreach($images as $image)
                        <tr>
                            <td>{{ $image->id }}</td>
                            <td><img src="{{ url('storage/items/'.$item->id.'/'.$image->file) }}" style="max-height: 100px;"></td>
                            <td>
                                

                                <a href="{{ route('admin.items.images.edit_status', ['item' => $item->id, 'image' => $image->id]) }}">
                                    @if ($image->active)
                            ...
                            <td>
                                ...
                                @if ($image->order_number > 1) 
                                    <a href="{{ route('admin.items.images.up', ['item' => $item->id, 'image' => $image->id]) }}">
                                        <i class="far fa-arrow-alt-circle-up"></i>
                                    </a>
                                    
                                @endif
                                @if ($image->order_number < $item->images->count())
                                    <a href="{{ route('admin.items.images.down', ['item' => $item->id, 'image' => $image->id]) }}">
            ...
            <form action="{{ route('admin.items.images.add', $item->id) }}" enctype="multipart/form-data" method="POST" role="form">
                @csrf
