...
@section('content')
    ...
    <div class="ibox-content">
        <div class="table-responsive">
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        @foreach ($images as $image)
                            <tr>
                                <td>{{ $image->id }}</td>
                                <td><img src="{{ url('storage/residences/' . $item->id . '/' . $image->file) }}"
                                        style="max-height: 100px;"></td>
                                <td>


                                    <a
                                        href="{{ route('admin.residences.images.edit_status', ['item' => $item->id, 'image' => $image->id]) }}">
                                        @if ($image->active)
                                ...
                                <td>
                                    ...
                                    @if ($image->order_number > 1)
                                        <a
                                            href="{{ route('admin.residences.images.up', ['item' => $item->id, 'image' => $image->id]) }}">
                                    ...
                                    @if ($image->order_number < $item->images->count())
                                        <a
                                            href="{{ route('admin.residences.images.down', ['item' => $item->id, 'image' => $image->id]) }}">
            ...
            <form action="{{ route('admin.residences.images.add', $item->id) }}" enctype="multipart/form-data"
