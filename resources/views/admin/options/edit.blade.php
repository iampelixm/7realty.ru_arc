...
@section('content')
    <div class="ibox-title">
        <h5>{{ __('admin.option_edit') }}</h5>
    </div>
    <div class="ibox-content">
        <form action="{{ route('admin.options.update', $option->id) }}" enctype="multipart/form-data" method="POST" role="form">
            @csrf
            @method('PUT')
           ...
           <div class="ibox ">
                        <div class="ibox-title">
                            <h5><i class="fa fa-cubes" aria-hidden="true"></i> Значение характиристики</h5>
                        </div>
                        <div class="ibox-content">
                                ...
                                <tbody>
                                    ...
                                    @if ($values != null)
                                        @foreach ($values as $key => $value)
                                            <tr id="valuerow{{ $key }}">
                                                <td>{{ $key }}</td>
                                                <td><input name="value[{{ $key }}]"  id="inpval{{ $key }}" type="text"  class="form-control form-control-sm" style="width:400px; background: #e4dbdb;" value="{{ $value }}" required></td>
                                                <td>
                                                    <button type="button" class="btn btn-outline btn-danger" onclick="deleterow('valuerow', {{ $key }})">Удалить</button>
                                                </td>
                                            </tr>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        function addvalue(){
            var inputprocent = "<input name=\"value["+indexvalue+"]\"  id=\"inpval"+indexvalue+"\" type=\"text\"  class=\"form-control form-control-sm\" style=\"width:400px; background: #e4dbdb;\" required>";
            var delbutton = "<button type=\"button\" class=\"btn btn-outline btn-danger\" onclick=\"deleterow('valuerow', "+indexvalue+")\">Удалить</button>"
            var str = "<tr id=\"valuerow"+indexvalue+"\"><td>"+indexvalue+"</td><td>"+inputprocent+"</td><td>"+delbutton+"</td></tr>";
