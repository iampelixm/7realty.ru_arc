    ...
    <div class="content-pdf">
      <table style="width: 80%;">
        <tr>
          ...
          <td style="font-size: 10px; text-align: left; border-left: 1px solid black; padding-left: 10px;" width="70%">
      ...
      <h3 class="content-pdf__h2 text-center" style="font-family: DejaVu Sans; font-size: 15px; color:grey; text-align: center;">Лот № {{ $item->id }}</h3>
      <p class="content-pdf__p text-center" style="text-align: center;" ><a href="#">@if ($item->slug) {{ route('site.item.get', $item->slug) }} @endif</a></p>
                ...
                @endif
                    <td style="padding: 10px; width: 48%;" >
                        <div class="content-pdf-image col">
                            <img class="content-pdf__img" src="{{ public_path('storage/items/'.$item->id.'/'.$image->file) }}" alt="info">
                        </div>
                    </td>
                @php 
                    $index = $key+1;
      ...
      <h3 class="content-pdf__h2 text-center" style="font-family: DejaVu Sans;  font-size: 15px; color:grey; text-align: center;">Цена</h3>
      <h2 class="content-pdf__h2 text-center" style="text-align: center;">{{ number_format($item->price, 0, ",", " ") }} руб.</h2>
              ...
              <ul class="row row-cols-2">
                  @if (isset($chank_item_options[0]))
                    @foreach ($chank_item_options[0] ?? [] as $key => $value)
                            <li class="col-6 py-2" style="font-weight: normal;">{{ $value->option_title }} - {{ $value->value_title }}</li>
                    @endforeach 
                  @endif
      ...
      <div class="content-pdf__h4 " style="font-weight: normal; text-align: justify; text-indent: 0px;">
        По вопросам получения дополнительной информации или уточнения информации, просим вас связаться с департаментом продаж компании <b>Seven</b> по телефону <a href="#">+7 985 700-00-77</a> или по email <a href="#">example@mail.com</a>
      </div>
    </div>
